// Hook Explanation
// This React component, specifically a custom hook named useQRCodeGenerator, 
// is designed to facilitate the generation and downloading of QR codes. 
// Initially, it utilizes the useState hook from React to manage three pieces of state: 
// url (to store the input URL that will be converted into a QR code), 
// qr (to store the generated QR code data URL), 
// and showInput (a boolean to toggle the visibility of an input element in the UI). 
// The hook exposes a method generateQRCode which utilizes the QRCode.toDataURL method 
// to convert the provided URL into a QR code, applying specific styling options, 
// and then updates the state with the generated QR code and hides the input.
// The downloadQRCode method allows users to download the generated QR code as a PNG file,
// prompting them to provide a filename and handling the download process 
// via creating an anchor element in the DOM. 
// Lastly, the repeatAction method resets the state to allow users to generate a new QR code. 
// The hook returns an object containing the state variables and methods,
// enabling them to be utilized in the component where the hook is used.
...
export const useQRCodeGenerator = () => {
  ...
  const generateQRCode = () => {
    ...
    // Use the Import of the qrcode and chain to the native method toDataUrl() 
    // much like the example provided and specify the data within the object 
    // that you will be passing such as the {url, {width, margin, color:{dark, light}}} 
    // which containes the information to generate the qr-code and url. 
    // Lastly, this native method toDataUrl() will contain a callback function 
    // that will update the qr variable and will also update the variable 
  ...
  const downloadQRCode = () => {
    ...
    const getFileName = () => {
      let fileName = prompt("Enter a file name for the QR code!")
  ...
  return { setUrl, url, setQr, qr, setShowInput, showInput, downloadQRCode, generateQRCode, repeatAction };
};
